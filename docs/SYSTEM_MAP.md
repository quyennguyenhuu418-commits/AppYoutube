...
│   ├── providers/
│   │   ├── base.py                132 lines — ABCs (LLM, TTS, Image, Search, Fetch)
...
│   │   ├── tts.py                 26 lines — get_tts_provider()
│   │   ├── elevenlabs_tts.py      87 lines — ElevenLabs provider
│   │   ├── gtts_tts.py            73 lines — Google TTS provider
...
├── tsconfig.json                 20 lines — ES2022, strict, noUnusedLocals=false
├── README.md                     31 lines — developer docs
└── src/
    ├── index.ts                  112 lines — CLI: loads job, runs renderMedia → MP4
    ├── Root.tsx                  63 lines — composition registry
    ├── lib/
    │   └── loadScene.ts          55 lines — loadSceneDefinition(jobDir)
    ├── compositions/
    │   └── Documentary.tsx       61 lines — main composition; maps scenes to Sequences
    ├── scenes/
